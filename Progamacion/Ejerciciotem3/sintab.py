...
while True:											#BUCLE PARA MANTENER EL PROGRAMA ACTIVO
	...
	opcion=input("Elija una opcion")							##EL USUARIO ELIGE QUE OPCION EJECUTAR Y LO GUARDA EN LA VARIABLE OPCION
												#SE CONVIERTE LA VARIABLE OPCION A INT
	try:											#DEPENDIENDO DEL NUMERO INTRODUCIDO SE EJECUTARA UNA OPCION					
...
#MENU DE LA OPCION DE COMPROBAR	
			...
			if opcion_comprobar == 1:
				...
				for i in range(len(Peliculas)):						#A TRAVES DEL FOR SE RECORRE LA LISTA, OBTENIENDO SU TAMAÑO CON LEN,DE MANERA QUE RECORRA
													#TODA LA LISTA SIENDO DEL TAMAÑO QUE SEA
				...
				if existe_serie == True:
					input("La serie ya esta en la lista existe")
				else :
					input("La serie no esta en la lista")
				...
				if existe_videojuego == True:
					input("El videojuego ya esta en la lista existe")
				else :
					input("EL videojuego no esta en la lista")
			else:
				assert 5 < 3		
